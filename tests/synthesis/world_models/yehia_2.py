...
def get_world_model():
    ...
    system_rules = {
        ...
        "transitions": [
            general_LTL("G( s1 <-> r1)", variables_str=["s1", "r1"], ap=ap),
            general_LTL("G( s2 <-> r2)", variables_str=["s2", "r2"], ap=ap),
            general_LTL("G( s3 <-> r3)", variables_str=["s3", "r3"], ap=ap),
            general_LTL("G( s4 <-> r4)", variables_str=["s4", "r4"], ap=ap),
            general_LTL("G( s5 <-> r5)", variables_str=["s5", "r5"], ap=ap),
        ],
        "constraints": [
